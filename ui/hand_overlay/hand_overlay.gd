...
@export var mouse_device_offset: int = 1
...
func _fill_lost_hands(tips: Array) -> Array:
	...
	for hand_data in tips:
		...
		var hand_key: Variant = hand_dict.get("hand_label", hand_index)
		seen_hands[hand_key] = true
		_last_seen_ms_by_hand[hand_key] = now_ms
		_last_tip_by_hand[hand_key] = hand_dict.duplicate(true)
	...
	for hand_key in _last_tip_by_hand.keys():
		...
		if now_ms - last_seen_ms <= lost_detection_hold_ms:
			var held_tip: Dictionary = _last_tip_by_hand[hand_key]
			merged.append(held_tip)
		else:
...
func _process_clicks() -> void:
	...
	for hand_data in _tips:
		...
		var pinch_distance_px: float = float(hand_dict.get("pinch_distance_px", 99999.0))
		var is_pinch := pinch_distance_px <= pinch_threshold_px
		var was_pinch := bool(_pressed_by_hand.get(hand_key, false))
		var middle_pos := (thumb_pos + index_pos) * 0.5
		...
		_update_mouse_position(middle_pos, hand_key, hand_index)

		_pressed_by_hand[hand_key] = is_pinch
	...
	for key in _pressed_by_hand.keys():
		if not seen_hands.has(key):
			stale.append(key)
	for key in stale:
		if bool(_pressed_by_hand.get(key, false)):
			var release_pos: Vector2 = _pos_by_hand.get(key, Vector2.ZERO)
			var release_index: int = int(_last_tip_by_hand.get(key, Dictionary()).get("hand_index", -1))
			if release_index >= 0:
				_emit_click_release(release_pos, release_index)
		_pressed_by_hand.erase(key)
		_pos_by_hand.erase(key)
		_last_seen_ms_by_hand.erase(key)
		_last_tip_by_hand.erase(key)
...
func _emit_click_press(screen_pos: Vector2, hand_index: int) -> void:
	if trigger_mouse_click:
		...
		mouse_click.device = _mouse_device_id(hand_index)
...
func _emit_click_release(screen_pos: Vector2, hand_index: int) -> void:
	if trigger_mouse_click:
		...
		mouse_release.device = _mouse_device_id(hand_index)
...
func _update_mouse_position(screen_pos: Vector2, hand_key: Variant, hand_index: int) -> void:
	if _pos_by_hand.get(hand_key, Vector2.ZERO).distance_to(screen_pos) > 10.0:
		var mouse_motion := InputEventMouseMotion.new()
		mouse_motion.device = _mouse_device_id(hand_index)
		mouse_motion.position = screen_pos
		mouse_motion.global_position = screen_pos
		_pos_by_hand[hand_key] = screen_pos
		Input.parse_input_event(mouse_motion)

func _mouse_device_id(hand_index: int) -> int:
	return int(max(0, mouse_device_offset + hand_index))
